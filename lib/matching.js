...
export async function createMatch(earlierListingId, laterListingId, initiatorUserId) {
  ...
  try {
    var result = await db.$transaction(async function(tx) {
      ...
      // Access control: the initiator MUST own one of the two listings.
      // Without this check, any authenticated user could force a match between
      // two strangers — locking their listings and triggering swap emails (IDOR / DoS).
      ...
      // A listing cannot be matched with itself / the same owner cannot be both sides.
    ...
    // The match + listing locks are already committed. A notification-email
    // failure must NOT make the whole match appear to fail, so swallow it here
    // (the match is still valid and visible in both dashboards).
    try {
...
export async function declineMatch(matchId, userId) {
  ...
  if (!match) throw new Error("Match not found");
  // Access control: the user must be one of the two parties in this match.
  // Without this, any authenticated user could decline a match between
  // strangers and unlock their listings (IDOR / griefing).
