...
export async function getCurrentUser() {
  ...
  const user = await db.user.findUnique({
    ...
    //
    // tokenVersion, registrationPaidAt and the Stripe ids are selected because
    // server code needs them, but they are stripped before this object reaches
    // a browser. See the publicUser() below.
    select: {
...
//
// /api/auth/me used to return the whole record, including tokenVersion and the
// raw Stripe customer and subscription ids. None of them have any client use:
// tokenVersion and registrationPaidAt are read only by server code, the
// customer id is used nowhere at all, and the subscription id was only ever
// tested for truthiness to decide between "Start free trial" and "Rejoin".
//
// Not a leak, since it is the viewer's own data, but there is no reason to put
// billing identifiers in reach of any script running on the page. The boolean
// hasSubscribedBefore carries the only fact the UI actually needed.
export function publicUser(user) {
  ...
  return {
    ...
    lifetimeFreeAccess: user.lifetimeFreeAccess,
    hasSubscribedBefore: Boolean(user.stripeSubscriptionId),
  };
}
