...
export function ErrorBox({ errors }) {
  ...
  return (
    <div role="alert" className="bg-red-950/60 border border-red-900/40 text-red-300 px-4 py-3 rounded-lg text-sm mb-5 leading-relaxed">
      {list.map((e, i) => (
...
export function SuccessBanner({ children }) {
  return (
    <div role="status" className="bg-emerald-950/60 border border-emerald-800/40 text-emerald-300 px-4 py-3 rounded-lg text-sm mb-5 flex items-center gap-2">
...
export function Field({ label, hint, required, children }) {
  ...
  return (
    <div>
      <label htmlFor={id} className="block text-xs text-[#9c9a92] mb-1">
        {label}
...
export function SecondaryButton({ children, onClick, className = "" }) {
  return (
    <button
      onClick={onClick}
      className={`px-5 py-2.5 rounded-lg border border-[#2a2a27] bg-transparent text-[#9c9a92] hover:border-[#444] hover:text-[#ccc] text-sm transition [touch-action:manipulation] ${className}`}
    >
      {children}
    </button>
  );
...
export function Countdown({ deadline, onExpired }) {
  ...
  return (
    <div className="flex items-center gap-3 mt-2">
      <div aria-hidden="true" className="flex-1 h-1.5 rounded-full bg-[#1a1a18] overflow-hidden">
        <div
...
export function PageShell({ children, title, subtitle, back, backLabel = "← Back", badge, maxWidth = "max-w-lg" }) {
  return (
    <div className={`${maxWidth} mx-auto px-5 py-10`}>
      {back && (
        <button onClick={back} className="inline-flex items-center min-h-[44px] text-[#73726c] hover:text-[#aaa] text-sm mb-5 transition [touch-action:manipulation]">
          {backLabel}
        </button>
      )}
      {badge}
...
export function StatCard({ label, value, sub }) {
  return (
    <div className="bg-[#1a1a18] rounded-lg border border-[#2a2a27] px-4 py-3">
      ...
      <div className="tabular-nums text-xl font-semibold text-[#e8e6dc]">{value}</div>
