...
export async function sendActionNeededEmail(recipient, match) {
  ...
  var content = greeting(recipient.name)
    ...
    + para("You're one step away. Open the swap to accept and unlock each other's contact details so you can arrange it.")
    + primaryButton(url, "Complete the swap")
    + divider()
    + smallText("This swap will expire if it isn't completed in time.");
  await getResend().emails.send({ from: getFROM(), to: recipient.email, subject: "Your swap partner agreed — one step left", html: emailWrapper(content) });
}
