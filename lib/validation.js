...
export function validateRegistration({ name, email, phone, password }) {
  ...
  // Email
  // The character class excludes < > " ' as well as whitespace and @. The
  // original pattern allowed them, which meant an address could carry markup
  // into the swap-confirmation email that a partner receives. Escaping at
  // render is the real defence (see lib/email.js); this stops it being stored
  // in the first place. No legitimate address contains these characters.
  if (!email || !email.trim()) {
