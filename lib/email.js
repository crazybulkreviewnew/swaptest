...
// Sent before the first payment is taken, off Stripe's trial_will_end event.
//
// Charging somebody who has forgotten they signed up is how you earn a
// chargeback and a bad review at the same time. This says the date, the amount
// and how to stop it, in that order, and does not try to talk them out of
// cancelling. £1 is not worth a surprise.
export async function sendTrialEndingEmail(user, endsAt) {
  var content = greeting(user.name)
    + heading("Your free trial ends " + fmtDate(endsAt))
    + para("This is just so it is not a surprise. Your SwapTest trial ends on <strong>" + fmtDate(endsAt) + "</strong>, and the first payment of <strong>£1</strong> will be taken that day. It is £1 a week after that.")
    + para("You do not need to do anything if you are happy to continue.")
    + divider()
    + para("<strong>If you would rather not continue,</strong> cancel before " + fmtDate(endsAt) + " and you will not be charged anything at all. It takes a few seconds from your dashboard, and you keep access until the trial runs out either way.")
    + primaryButton(getAPP_URL() + "/dashboard", "Manage my membership")
    + divider()
    + smallText("Listing a test and accepting a swap somebody offers you are always free. The membership only covers asking other learners for an earlier date.");

  await getResend().emails.send({
    from: getFROM(), to: user.email,
    subject: "Your SwapTest trial ends " + fmtDate(endsAt) + " — £1 will be taken",
    html: emailWrapper(content),
  });
}
...
export async function sendWelcomeEmail(user) {
  ...
  var content = greeting(user.name)
    ...
    // Uses paymentsEnabled() rather than reading the env var directly, so this
    // cannot disagree with the rest of the app about whether payments are on.
    // It also described the retired £1 registration and £8 swap fees, telling
    // brand new users about charges that no longer exist.
    + para(paymentsEnabled()
        ? "If you are happy to move your test to a <strong>later</strong> date, SwapTest is completely <strong>free</strong>: listing, matching and swapping all cost nothing. If you want an <strong>earlier</strong> date, a membership is <strong>£1 a week</strong> and starts with a <strong>" + TRIAL_DAYS + " day free trial</strong>. Cancel any time from your dashboard."
