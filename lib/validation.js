...
// Parse a date input as LOCAL midnight rather than UTC midnight, so date-only
// comparisons don't shift by a day depending on the server's timezone.
// Rejects out-of-range parts ("2026-13-45") instead of letting Date roll them
// over into a valid-looking date.
function parseLocalDate(value) {
  const ymd = typeof value === "string" && value.match(/^(\d{4})-(\d{2})-(\d{2})/);
  if (!ymd) return new Date(value); // let the caller's isNaN check handle it
  const year = Number(ymd[1]), month = Number(ymd[2]), day = Number(ymd[3]);
  const d = new Date(year, month - 1, day);
  const rolledOver =
    d.getFullYear() !== year || d.getMonth() !== month - 1 || d.getDate() !== day;
  return rolledOver ? new Date(NaN) : d;
}
...
export function validateListing({
  ...
  // Parse "YYYY-MM-DD" (what <input type="date"> submits) as a LOCAL date.
  // `new Date("2026-07-24")` is parsed as UTC midnight, which lands on a
  // different calendar day than local midnight wherever the server isn't on
  // UTC — under British Summer Time that made "must be after today" accept a
  // test booked for today. Comparing two local midnights is timezone-stable.
  const parsedCurrent = parseLocalDate(currentDate);
