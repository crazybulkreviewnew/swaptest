...
export async function sendSwapRequestEmail(match, responder, initiatorListing, responderListing) {
  ...
  var content = greeting(responder.name)
    ...
    + urgentBadge("You have 24 hours to respond. After that, this request will expire.")
...
export async function sendActionNeededEmail(recipient, match) {
  ...
  var content = greeting(recipient.name)
    ...
    + urgentBadge("You have 24 hours to complete this swap. After that, it will expire.")
...
// Nudge sent as the deadline approaches to whoever hasn't agreed yet.
// The whole point is that the first email can be missed, so this repeats the
// full swap details rather than assuming they read the original.
export async function sendSwapReminderEmail(recipient, match, hoursLeft) {
  var url = getAPP_URL() + "/match?id=" + match.id;
  var recipientIsEarlier = recipient.id === match.earlierUserId;
  var recipientListing = recipientIsEarlier ? match.earlierListing : match.laterListing;
  var partnerListing = recipientIsEarlier ? match.laterListing : match.earlierListing;
  var timeLeft = hoursLeft === 1 ? "1 hour" : hoursLeft + " hours";
  var content = greeting(recipient.name)
    + heading("Don't lose this swap")
    + urgentBadge("About " + timeLeft + " left to agree, or this match expires.")
    + para("Someone wants to swap driving test dates with you and is waiting on your answer. If you don't respond, the match expires and both tests go back into the pool.")
    + detailsCard([
      { label: "Your current test", value: fmtDate(recipientListing.currentDate) + " at " + recipientListing.currentTime },
      { label: "Your test centre", value: recipientListing.centre },
      { label: "You would get", value: fmtDate(partnerListing.currentDate) + " at " + partnerListing.currentTime, color: "#1D9E75" },
    ...
    + primaryButton(url, "Agree to the swap")
    + divider()
    + smallText("Not interested? You can decline on the same page, which frees both tests up straight away.");
  await getResend().emails.send({
    from: getFROM(), to: recipient.email,
    subject: "Reminder: " + timeLeft + " left to confirm your test swap",
...
export async function sendContactExchange(match) {
  ...
  var content1 = greeting(earlierUser.name)
    ...
    + para("Here are your swap partner's details. Get in touch with them, then contact DVSA to arrange swapping your test dates with each other.")
...
export async function sendExpiredNotification(match, reason) {
  var earlierUser = match.earlierUser; var laterUser = match.laterUser;
  var c = function(user) {
    return greeting(user.name) + heading("Your swap match has expired") + warningBadge("The response window has passed.")
...
export async function sendWelcomeEmail(user) {
  ...
  var content = greeting(user.name)
    ...
    + para(process.env.NEXT_PUBLIC_PAYMENTS_ENABLED === "true"
        ? "Membership is <strong>£1 a week</strong> and covers listing your test, viewing your matches and as many swaps as you need. There is no separate swap fee, and you can cancel at any time."
