...
export async function findMatches(earlierListing) {
  ...
  var matches = await db.listing.findMany({
    ...
    // only the swap-relevant listing fields are returned.
    orderBy: { currentDate: "asc" },
  });
  return matches.filter(function(m) {
    return canSwapCentres(earlierListing.centre, m.centre);
  });
}
...
export async function findMatchesForLater(laterListing) {
  ...
  return matches.filter(function(m) {
    return canSwapCentres(laterListing.centre, m.centre);
  });
}
...
export async function recordConsent(matchId, userId) {
  ...
  if (!result.completed) {
    try {
      var m = await db.match.findUnique({ where: { id: matchId }, include: { earlierUser: true, laterUser: true } });
...
export async function completeSwapPayment(matchId, payingUserId, stripePaymentId) {
  ...
  if (!result.completed) {
    try {
      var m = await db.match.findUnique({ where: { id: matchId }, include: { laterUser: true } });
