...
// Where Stripe should send somebody back to.
//
// NEXT_PUBLIC_APP_URL is the production domain, deliberately: it is what the
// links inside emails must use. But building checkout return URLs from it means
// paying on localhost lands you on the live site, signed out, staring at a login
// page — which is exactly what happened on the first real test.
//
// So prefer the origin the request actually came from, and only trust it if it
// is localhost or the configured app URL. Anything else (a spoofed Host header)
// falls back to the configured value, so this cannot be used to bounce somebody
// to an attacker's page after payment.
export function safeReturnOrigin(requestUrl) {
  var configured = process.env.NEXT_PUBLIC_APP_URL || "";
  if (!requestUrl) return configured;
  try {
    var origin = new URL(requestUrl).origin;
    var host = new URL(origin).hostname;
    if (host === "localhost" || host === "127.0.0.1") return origin;
    if (configured && origin === new URL(configured).origin) return origin;
  } catch (e) { /* fall through */ }
  return configured;
}

export async function createSubscriptionCheckoutSession({ userId, userEmail, stripeCustomerId, amountPence, trialDays, appUrl }) {
...
export async function createBillingPortalSession({ stripeCustomerId, returnPath, appUrl }) {
  return getStripe().billingPortal.sessions.create({
    customer: stripeCustomerId,
    return_url: (appUrl || process.env.NEXT_PUBLIC_APP_URL || "") + (returnPath || "/dashboard"),
  });
}
